...
@Component({
    selector: 'contact-type-selection',
    templateUrl: './contact-type-selection.component.html',
    styleUrls: ['./contact-type-selection.component.sass'],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => ContactTypeSelectionComponent),
            multi: true,
        },
    ],
})
export class ContactTypeSelectionComponent implements ControlValueAccessor, OnInit {
     ...
     */
    set value(val) {
        console.log(`ContactType set to: ${val}`);
        this._value = val;
        this.onChange(val);
        this.onTouched();
    }
